...
## Phase 11 routes (analytics) and the generated document

- `GET /api/v1/admin/analytics?range=` — platform-wide. Requires `tenant:manage_all`
  (SUPER_ADMIN only); runs with no tenant context by design.
- `GET /api/v1/tenant/analytics?range=` — the caller's clinic. The tenant id comes from the
  session, never the request.
- `GET /api/v1/tenant/analytics/export?range=` — operational appointment CSV. No clinical
  content; the export is audited with a row count.
- `GET /api/v1/doctor/analytics?range=` and `GET /api/v1/rep/analytics?range=` — the
  caller's own performance, scoped by a profile resolved from the session.

`range` is one of `7d | 30d | 90d | 12m`; anything else falls back to `30d`. Buckets are
UTC, which the UI states rather than leaving to be discovered.

### The OpenAPI document

`GET /api/v1/openapi` serves `public/openapi.json`, generated by `npm run openapi`.

This replaces the "OpenAPI: generated from the Zod schemas… (Phase 11)" note in Conventions
above. Request bodies are the actual Zod schemas the handlers validate with, so they cannot
drift. Generation **fails** if a handler has no entry in `src/lib/openapi/registry.ts`, or
if an entry names a handler that no longer exists — an API document that silently omits a
route is worse than none.

Response bodies are documented by status code and the standard envelope only. There are no
response Zod schemas (handlers return Prisma rows), and hand-writing approximations would
produce a document that lies in a way no tooling can catch.
